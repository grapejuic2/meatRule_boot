...
public class ServController extends BaseController{
	...
	//게시물 목록  +페이징
	    ...
	    public String postServWrite(ServVO vo, MultipartHttpServletRequest multipartRequest, HttpServletResponse response, RedirectAttributes redirectAttrs) throws Exception {
	        ...
	        try {
	        	System.out.println("컨트롤러 여기타고 있니 2");
		...
		@ResponseBody
		public String addReply(@RequestParam int cmt_parent_num,@ModelAttribute("CommentVO") CommentVO commentVO, HttpServletRequest request, HttpServletResponse response) throws Exception {
		    HttpSession session = request.getSession();
		    MemberVO memberVO = (MemberVO) session.getAttribute("memberInfo");
